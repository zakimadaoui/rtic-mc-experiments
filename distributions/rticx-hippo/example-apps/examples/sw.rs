...
pub mod app {
    ...
    #[inline]
    pub fn __rticx_interrupt_free<F, R>(f: F) -> R
    where
        F: FnOnce() -> R,
    {
        ...
        let r = f();
        unsafe {
            rticx_hippo::export::interrupt_enable();
        }
        r
    }
    /// ==================================== User code ======================================
    static mut __rticx_internal__Sw1__INPUTS: rticx_hippo::export::Queue<
        <Sw1 as RticSwTask>::SpawnInput,
        2,
    > = rticx_hippo::export::Queue::new();
    impl Sw1 {
        pub fn spawn(
            input: <Sw1 as RticSwTask>::SpawnInput,
        ) -> Result<(), <Sw1 as RticSwTask>::SpawnInput> {
            let mut inputs_producer = unsafe { __rticx_internal__Sw1__INPUTS.split().0 };
            let mut ready_producer = unsafe { __rticx_internal__Core0Prio1Tasks__RQ.split().0 };
            __rticx_interrupt_free(|| -> Result<(), <Sw1 as RticSwTask>::SpawnInput> {
                inputs_producer.enqueue(input)?;
                unsafe { ready_producer.enqueue_unchecked(Core0Prio1Tasks::Sw1) };
                __rticx_sc_pend(hippomenes_core::Interrupt2);
                Ok(())
            })
        }
    }
    ...
    static mut __rticx_internal__Core0Prio1Tasks__RQ: rticx_hippo::export::Queue<
        Core0Prio1Tasks,
        2usize,
    > = rticx_hippo::export::Queue::new();
    ...
    #[doc(hidden)]
    #[inline]
    pub fn __rticx_sc_pend<T: rticx_hippo::export::Interrupt>(irq_nbr: T) {
        rticx_hippo::export::pend(irq_nbr);
    }
    /// ====================================
    /// CORE 0
    /// ====================================
    ...
    impl SomeTask {
        const fn current_core() -> __rticx__internal__Core0 {
            unsafe { __rticx__internal__Core0::new() }
        }
    }
    ...
    impl RticTask for Core0Priority1Dispatcher {
        ...
        fn exec(&mut self) {
            unsafe {
                let mut ready_consumer = __rticx_internal__Core0Prio1Tasks__RQ.split().1;
                if let Some(task) = ready_consumer.dequeue() {
                    match task {
                        Core0Prio1Tasks::Sw1 => {
                            let mut input_consumer = __rticx_internal__Sw1__INPUTS.split().1;
    ...
    impl RticMutex for __uart_mutex {
        ...
        fn lock(&mut self, f: impl FnOnce(&mut Self::ResourceType)) {
            ...
            unsafe {
                rticx_hippo::export::lock(resource_ptr, task_priority as u8, CEILING as u8, f);
            }
        }
    }
    ...
    pub fn main() -> ! {
        __rticx_interrupt_free(|| {
            ...
            unsafe {
                rticx_hippo::export::enable(hippomenes_core::Interrupt0, 2u16 as u8);
                rticx_hippo::export::enable(hippomenes_core::Interrupt2, 1u16 as u8);
            }
        });
        loop {}
    }
