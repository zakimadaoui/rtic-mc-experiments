pub mod my_app
{
    ...
    } impl RticMutex for __led_mutex
    {
        ...
        {
            const CEILING : u16 = 2u16 ; let task_priority = self.priority ;
            let resource = unsafe
    ...
    #[doc =
    r"======================= define and bind sw tasks to dispatchers ======================"]
    static mut MY_TASK2 : core :: mem :: MaybeUninit < MyTask2 > = core :: mem
    :: MaybeUninit :: uninit() ; struct MyTask2 ; impl RticSwTask for MyTask2
    {
        type SpawnInput = u8 ; fn init() -> Self { Self } fn
        exec(& mut self, _input : u8) { self.shared().led.lock(| _led | {}) }
    ...
    new() ; impl MyTask2
    {
        ...
        Result < (), < MyTask2 as RticSwTask > :: SpawnInput > { Ok(()) }
    ...
    new() ; impl MyTask7
    {
        ...
        Result < (), < MyTask7 as RticSwTask > :: SpawnInput > { Ok(()) }
