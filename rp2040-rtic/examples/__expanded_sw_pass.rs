pub mod my_app
{
    ...
    rp2040_hal :: pac :: { self } ; type LedOutPin = Pin < Gpio25, FunctionSio
    < SioOutput >, PullDown > ; static DELAY : u32 = 1000 ; #[shared] struct
    SharedResources { alarm : Alarm0, led : LedOutPin, } #[init] fn
    system_init() -> SharedResources
    {
    ...
    } #[task(binds = TIMER_IRQ_0, priority = 3, shared = [alarm, led])] struct
    ...
    } #[task(binds = TIMER_IRQ_2, priority = 1, shared = [alarm])] struct
    MyTask3 ; impl RticTask for MyTask3
    { fn init() -> Self { Self } fn exec(& mut self) {} } #[idle] struct
    MyIdleTask { count : u32, } impl RticIdleTask for MyIdleTask
    {
    ...
    #[task(priority = 2, shared = [led])] struct MyTask2 ; impl RticSwTask for
    MyTask2
    {
    ...
    < MyTask2 as RticSwTask > :: SpawnInput, 2 > = rtic :: export :: Queue ::
    new() ; impl MyTask2
    {
    ...
    } #[task(priority = 2, shared = [led])] struct MyTask7 ; impl RticSwTask
    for MyTask7
    {
        type SpawnInput = u16 ; fn init() -> Self { Self } fn
        exec(& mut self, input : u16)
    ...
    Prio2Tasks, 3usize > = rtic :: export :: Queue :: new() ; #[doc(hidden)]
    #[task(binds = DMA_IRQ_0, priority = 2u16)] pub struct Priority2Dispatcher
